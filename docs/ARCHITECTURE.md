...
Neuron Wire Protocol (NWP) is a zero-copy, peer-to-peer infrastructure layer for decentralized AI. Its production peer transport is UDP: `src/transport.rs` implements custom reliable delivery with selective ACK bitfields, and `src/engine_loop.rs` drives that UDP socket from a single-threaded non-blocking event engine at ~1ms tick intervals with zero busy-wait. TCP in this repository is limited to legacy/tooling stream-framing helpers (`src/io.rs`) and HTTP-style observability endpoints; it is not the production NWP transport. Nodes authenticate via Ed25519 digital signatures, establish encrypted channels through static-static X25519 ECDH handshakes with XChaCha20-Poly1305 AEAD, and participate in a reputation-based trust system that provides Sybil resistance. The system supports distributed neural network computation including forward passes, Hebbian/STDP learning, gradient exchange, knowledge distillation, continual learning with meta-learning, and curiosity-driven exploration — all coordinated over a flat, zero-copy FlatBuffer wire format designed for constrained environments (512MB VPS).
